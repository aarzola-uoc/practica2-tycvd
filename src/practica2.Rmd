...
setwd("C:/Users/USER-PC/Desktop/practica2-tycvd/src")
...
Cada vez que un tiro sale del aro, hay cuatro resultados posibles: el balón podría salirse de los límites del campo y contarse como un rebote del equipo defensivo; la pelota podría rebotar de un jugador defensivo y salirse de los límites para ser contada como un rebote ofensivo del equipo; la pelota podría ser caputra por un jugador defensivo y contarse como un rebote defensivo; o un jugador ofensivo podría capturar la pelota y contarse como un rebote ofensivo.
...
* TO=Pérdidas.
* TMP=Minutos jugados por equipo.
* MP=Minutos jugados.
* TFGA=Tiros de campo intentados del equipo. 
* TFTA=Tiros libres intentados del equipo.
* TTO=Pérdidas del equipo.
...
Miden a los jugadores idividualistas y relacionan lo que generan en relación a la vez que acaban la jugada. Esto divide la cantidad de asistencias que tiene un jugador por la cantidad de posesiones que terminan en las manos de ese jugador.
...
265/5000
Porcentaje de tiros "taponables" de los oponentes que el jugador bloquea (elimina los tiros de 3 puntos, ya que estos generalmente no son "taponables", algo arbitrario ya que los ves bloqueados y es poco probable que un tiro de salto de medio alcance sea bloqueado, pero es incluido en la muestra)
...
Para realizar una comprobación más visual de la normalidad de las variables, vamos a utilizar _Q-Q Plots_. Los gráficos cuantil-cuantil permiten comparar la distribución de un conjunto de datos con la de otro conjunto de datos o con una distribución ideal teórica. En nuestro caso utilizaremos la función `qnorm()` para comprar los valores de cada una de nuestras variables cuantitativas con la distribución normal.

```{r ,eval=TRUE,echo=TRUE}
par(mfrow=c(2,2))
for(i in 2:ncol(data_team)) {
  if (is.numeric(data_team[,i])){
    qqnorm(data_team[,i],main = paste("Normal Q-Q Plot for ",colnames(data_team)[i]))
    qqline(data_team[,i],col="red")
    hist(data_team[,i], 
      main=paste("Histogram for ", colnames(data_team)[i]), 
      xlab=colnames(data_team)[i], freq = FALSE)
  }
}
par(mfrow=c(2,2))
```

En base a los diagramas obtenidos, podemos determinar que aunque la mayoría de variables no siguen una distribución normal, si fuera necesario podrían normalizarse, salvo para las variables `Min` y `Local`.
...
```{r,eval=TRUE,echo=TRUE}
model_offensive_rating<- lm(Off_Rat~., data=data_ataque)
summary(model_offensive_rating)
```
...
**Predicciones para las medias por equipo**

Vamos a realizar la predicción por equipo y visualizar su media con respecto a lo observado.

Comencemos por el rating ofensivo:

```{r,eval=TRUE,echo=TRUE}
dt_data_teamopp <- data.table(data_teamopp)
acumulados_team<-dt_data_teamopp[,list(Off_Rat=mean(Off_Rat),
eFG=mean(eFG),
Off_Reb=mean(Off_Reb),
Pct_To=mean(Pct_To),
FTR=mean(FTR),
Def_Rat=mean(Def_Rat),
eFG_Opp=mean(eFG_Opp),
Off_Reb_Opp=mean(Off_Reb_Opp),
Pct_To_opp=mean(Pct_To_opp),
FTR_opp=mean(FTR_opp)),by=Team]

data_offteam<-acumulados_team[,c("Team","Off_Rat")]

data_offteam$Off_Rat_predicted <- as.data.frame(predict(model_offensive_rating, acumulados_team))

data_offteam2 <- melt(data_offteam, id.vars='Team')

ggplot(data_offteam2, aes(x=Team, y=value, fill=variable)) +
    geom_bar(stat='identity', position='dodge')+ coord_cartesian(ylim=c(0.8,1.25))+ 
  theme(axis.text.x = element_text(angle = 90))
```
Como podíamos preveer, la estimación es realmente buena. 

Analicemos ahora el rating defensivo:

```{r,eval=TRUE,echo=TRUE}
data_defteam<-acumulados_team[,c("Team","Def_Rat")]

data_defteam$Def_Rat_predicted <- predict(model_defensive_rating, acumulados_team)


data_defteam2 <- melt(data_defteam, id.vars='Team')

ggplot(data_defteam2, aes(x=Team, y=value, fill=variable)) +
    geom_bar(stat='identity', position='dodge')+ coord_cartesian(ylim=c(0.8,1.25))+ 
  theme(axis.text.x = element_text(angle = 90))
```
De nuevo observamos que la estimación es realmente buena con respecto a la media real.
...
## Aplicación
Vamos a analizar el porcentaje de probabilidad de victoria del Real Madrid contra el resto de equipos jugando de local y de visitante en función de sus datos medios observados.

Cada variable se informará con la media de lo realizado por el conjunto blanco y por su rival. Así por ejemplo, el eFG que se le imputará en su partido contra el Anadolu Efes será:
- eFG = Mean(eFG_{Real Madrid}, eFG\_Opp_{Anadolu Efes})

```{r,eval=TRUE,echo=TRUE}
df1<-acumulados_team[acumulados_team$Team == "Real Madrid"]
df2<-acumulados_team[acumulados_team$Team != "Real Madrid"]
df_RM_local<-matrix(ncol=10, nrow=nrow(df2))
colnames(df_RM_local)<-c("Team","eFG","eFG_Opp","Off_Reb","Pct_To","FTR","Off_Reb_Opp","Pct_To_opp","FTR_opp","Local")
df_RM_local<-as.data.frame(df_RM_local)
for (i in 1:nrow(df2)){
  df_RM_local$Team[i] <- as.character(df2$Team[i])
  df_RM_local$eFG[i]  <- 0.5*(df1$eFG+df2$eFG_Opp[i])      
  df_RM_local$Off_Reb[i] <- 0.5*(df1$Off_Reb+df2$Off_Reb_Opp[i])  
  df_RM_local$Pct_To[i] <- 0.5*(df1$Pct_To+df2$Pct_To_opp[i])    
  df_RM_local$FTR[i]  <- 0.5*(df1$FTR+df2$FTR_opp[i])      
  df_RM_local$eFG_Opp[i]  <- 0.5*(df1$eFG_Opp+df2$eFG[i]) 
  df_RM_local$Off_Reb_Opp[i]<- 0.5*(df1$Off_Reb_Opp+df2$Off_Reb[i])
  df_RM_local$Pct_To_opp[i] <- 0.5*(df1$Pct_To_opp+df2$Pct_To[i])
  df_RM_local$FTR_opp[i]   <- 0.5*(df1$FTR_opp+df2$FTR[i])
  df_RM_local$Local[i]   <-1
}

df_RM_visit<-matrix(ncol=10, nrow=nrow(df2))
colnames(df_RM_visit)<-c("Team","eFG","eFG_Opp","Off_Reb","Pct_To","FTR","Off_Reb_Opp","Pct_To_opp","FTR_opp","Local")
df_RM_visit<-as.data.frame(df_RM_visit)
for (i in 1:nrow(df2)){
  df_RM_visit$Team[i] <- as.character(df2$Team[i])
  df_RM_visit$eFG[i]  <- 0.5*(df1$eFG+df2$eFG_Opp[i])      
  df_RM_visit$Off_Reb[i] <- 0.5*(df1$Off_Reb+df2$Off_Reb_Opp[i])  
  df_RM_visit$Pct_To[i] <- 0.5*(df1$Pct_To+df2$Pct_To_opp[i])    
  df_RM_visit$FTR[i]  <- 0.5*(df1$FTR+df2$FTR_opp[i])      
  df_RM_visit$eFG_Opp[i]  <- 0.5*(df1$eFG_Opp+df2$eFG[i]) 
  df_RM_visit$Off_Reb_Opp[i]<- 0.5*(df1$Off_Reb_Opp+df2$Off_Reb[i])
  df_RM_visit$Pct_To_opp[i] <- 0.5*(df1$Pct_To_opp+df2$Pct_To[i])
  df_RM_visit$FTR_opp[i]   <- 0.5*(df1$FTR_opp+df2$FTR[i]) 
  df_RM_visit$Local[i]   <-0
}



df_RM_local$Win_Prob_home <- predict(model_victoria, df_RM_local,type="response")


df_RM_visit$Win_Prob_visit <- predict(model_victoria, df_RM_visit,type="response")
```

Analizamos ahora visualmente los resultados obtenidos:
```{r,eval=TRUE,echo=TRUE}
df_RM_probwin<-matrix(ncol=3, nrow=nrow(df2))
colnames(df_RM_probwin)<-c("Team","Prob Win Home", "Prob Win Visit")
df_RM_probwin<-as.data.frame(df_RM_probwin)

df_RM_probwin$Team <- df_RM_local$Team 
df_RM_probwin$`Prob Win Home` <- df_RM_local$Win_Prob_home
df_RM_probwin$`Prob Win Visit` <- df_RM_visit$Win_Prob_visit
df_RM_probwin2 <- melt(df_RM_probwin, id.vars='Team')

ggplot(df_RM_probwin2, aes(x=Team, y=value, fill=variable)) +
    geom_bar(stat='identity', position='dodge')+
  theme(axis.text.x = element_text(angle = 90))
```

Así vemos que, con las medias del Real Madrid lo normal sería que ganara casi todos los partidos a excepción de Efes, el líder de la competición y equipo muy fuerte y Maccabi, Barcelona y CSKA cuando jueguen estos en casa (equipos también muy fuertes).

Resultados lógicos y acordes con lo modelado.
...
print(as.character(head(df2$nombre[df2$row!=df2$col],5)))
```
...
a1 <- subset(data_players, PlayerName %in% as.character(head(df2$nombre[df2$row!=df2$col],5)))
b1 <- as.data.frame(select(a1, PlayerName, Steal_pct, Block_pct, Offensive_Reb_pct, Defensive_Reb_pct, TS_player))
b1[is.na(b1)] <- 0
c1 <- as.data.frame(data.table(b1)[, .(mean(Steal_pct*100), 
                                       mean(Block_pct*100),
                                       mean(Offensive_Reb_pct*100),
                                       mean(Defensive_Reb_pct*100),
                                       mean(TS_player*100)),by = PlayerName])
posicion <- rep('Base', length(c1$PlayerName))
c1 <- cbind(c1, posicion)
names(c1) <- c("individual","STL%","BLK%","ORB%","DRB%", "TS%","group")
c1 <- c1[, c(1,7,2,3,4,5,6)]
...
c1 <- as.data.frame(data.table(b1)[, .(mean(Steal_pct*100), 
                                       ...
                                       mean(Defensive_Reb_pct*100),
                                       mean(TS_player*100)),by = PlayerName])
posicion <- rep('Escolta', length(c1$PlayerName))
c1 <- cbind(c1, posicion)
names(c1) <- c("individual","STL%","BLK%","ORB%","DRB%", "TS%","group")
c1 <- c1[, c(1,7,2,3,4,5,6)]
...
En este caso nos recomienda de nuevo a jugadores del perfil anotador de Larkin. John Dibartolomeo, Jimmer Fredette, James Nunally,
Adreian Payne y Brock Tum.
...
c1 <- as.data.frame(data.table(b1)[, .(mean(Steal_pct*100), 
                                       ...
                                       mean(Defensive_Reb_pct*100),
                                       mean(TS_player*100)),by = PlayerName])
posicion <- rep('Alero', length(c1$PlayerName))
c1 <- cbind(c1, posicion)
names(c1) <- c("individual","STL%","BLK%","ORB%","DRB%", "TS%","group")
c1 <- c1[, c(1,7,2,3,4,5,6)]
...
Como alternativas a Gigi Datome, nos presenta Ron Baker, Petteri Koponen, Jaycee Carroll, Aaron Doornekamp y Johann Digtmann. Aleros con características de tirador. 
...
c1 <- as.data.frame(data.table(b1)[, .(mean(Steal_pct*100), 
                                       ...
                                       mean(Defensive_Reb_pct*100),
                                       mean(TS_player*100)),by = PlayerName])
posicion <- rep('Ala Pívot', length(c1$PlayerName))
c1 <- cbind(c1, posicion)
names(c1) <- c("individual","STL%","BLK%","ORB%","DRB%", "TS%","group")
c1 <- c1[, c(1,7,2,3,4,5,6)]
...
Como opción a Livio Jean-Charles: Bojan Dubljevic, Lukas Lekavicius, Livio Jean-Charles, Cory Higgins y Ilimane Diop.
...
c1 <- as.data.frame(data.table(b1)[, .(mean(Steal_pct*100), 
                                       ...
                                       mean(Defensive_Reb_pct*100),
                                       mean(TS_player*100)),by = PlayerName])
posicion <- rep('Pívot', length(c1$PlayerName))
c1 <- cbind(c1, posicion)
names(c1) <- c("individual","STL%","BLK%","ORB%","DRB%", "TS%","group")
c1 <- c1[, c(1,7,2,3,4,5,6)]
...
a Ahmet Duveriouglu, Kyle Hines, Vladimir Stimac. Jan Vesely y Agustine Rubit.
...
En la siguiente visualización vamos a representar cinco estadísticas de cada jugador divididos por posición. Los jugadores seleccionados para cada posición son los que se han obtenido como jugadores recomendados en el apartado de [\textcolor{blue}{clustering de jugadores}](#clustering-de-jugadores). Se ha elegido para esta visualización una gráfica de barras en forma circular con las siguientes estadísticas:

* Porcentaje de bloqueos (BLK%)
* Porcentaje de rebotes defensivos (DRB%)
* Porcentaje de rebotes ofensivos (ORB%)
* Porcentaje de robos (STL%)
* Porcentaje de tiros reales (TS%)

La fórmula para obtener estos estadísticas y su explicación se encuentran en el [\textcolor{blue}{apartado 4.3}](#creación-de-nuevas-métricas-de-jugadores).

```{r, eval=TRUE, echo=FALSE, fig.width=10, fig.height=10, warning=FALSE}
...
q <- ggplot(data) +      
  ...
  ggplot2::annotate("text", x = rep(max(data$id),5), y = c(0, 20, 40, 60, 80), label = c("0", "20", "40", "60", "80") , color="grey", size=6 , angle=0, fontface="bold", hjust=1) +
  ...
  geom_text(data=label_data, aes(x=id, y=10, label=individual, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=2.5, angle= label_data$angle, inherit.aes = FALSE ) +
  
  # Add base line information
  geom_segment(data=base_data, aes(x = start, y = -5, xend = end, yend = -5), colour = "black", alpha=0.8, size=0.6 , inherit.aes = FALSE )  +
  geom_text(data=base_data, aes(x = title, y = -10, label=group), hjust=c(1,1,1,0,0), colour = "black", alpha=0.8, size=2, fontface="bold", inherit.aes = FALSE)

q
```

Al igual que en la visualización anterior se han ocultado las distintas opciones de configuración del gráfico para facilitar la lectura de este informe. No obstante, se pueden consultar en el fichero `practica2.Rmd` con el código fuente desde el que se ha generado este pdf.

En cuanto al gráfico, vemos que las cinco posiciones están bastante igualadas en base a estos jugadores y estos estadísticos, pero si podemos ver como el mayor porcentaje de bloqueos lo tienen los pívots y ala pívot. También se ve bastante claro que los bases son los que menor porcentaje de rebotes tienen.
...
* La dinámica obviamente está cambiando y el ejemplo de la NBA es el más inmediato, donde todas las franquicias tienen un equipo de más de treinta personas trabjaando en Big Data & Advanced Analytics dando soporte a las decisiones.
