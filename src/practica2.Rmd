...
* **Variables sin _outliers_:** `MatchId`, `Fv`, `Ag`, `Cm` y `PIR`.
...
```{r}
data_players2$Assist_pct <- data_players2$As.x/(((data_players2$Min/(data_players2$Min.x/5))*(data_players2$x2A.y+data_players2$x3A.y))-(data_players2$x2A.x+data_players2$x3A.x))
```
...
```{r}
data_players2$Turnover_pct<-ifelse((data_players2$x2A.x+data_players2$x3A.x+0.44*data_players2$FTA.x+data_players2$To.x)==0, 0,data_players2$To.x/(data_players2$x2A.x+data_players2$x3A.x+0.44*data_players2$FTA.x+data_players2$To.x))
```
...
```{r}
data_players2$Offensive_Reb_pct <- data_players2$O.x*(data_players2$Min.x/5)/(data_players2$Min*(data_players2$O.y+data_players2$D_Opp))

data_players2$Defensive_Reb_pct <- data_players2$D.x*(data_players2$Min.x/5)/(data_players2$Min*(data_players2$D.y+data_players2$O_Opp))
...
```{r}
data_players2$eFG_player<-ifelse((data_players2$x3A.x+data_players2$x2A.x)==0,0,(data_players2$x2M.x+1.5*data_players2$x3M.x)/(data_players2$x3A.x+data_players2$x2A.x))
```
...
```{r}
data_players2$TS_player <- ifelse((2*(data_players2$x3A.x+data_players2$x2A.x+0.44*data_players2$FTA.x))==0,0,data_players2$Pts.x/(2*(data_players2$x3A.x+data_players2$x2A.x+0.44*data_players2$FTA.x)))
```
...
(data_players2$x3A.x+data_players2$x2A.x+0.44*data_players2$FTA.x+data_players2$To.x)*(data_players2$Min.x/5)/(data_players2$Min*(data_players2$x3A.y+data_players2$x2A.y+0.44*data_players2$FTA.y+data_players2$To.y))
...
```{r}
data_players2$Points_per_possesion <-ifelse((data_players2$x3A.x+data_players2$x2A.x+0.44*data_players2$FTA.x+data_players2$To.x)==0,0, data_players2$Pts.x/(data_players2$x3A.x+data_players2$x2A.x+0.44*data_players2$FTA.x+data_players2$To.x))
```
...
```{r}
data_players2$FTR_player <- ifelse((data_players2$x2A.x+data_players2$x3A.x)==0,0,data_players2$FTM.x/(data_players2$x2A.x+data_players2$x3A.x))
```
...
```{r}
data_players2$Hollinger_Assist_ratio <- ifelse((data_players2$x2A.x+data_players2$x3A.x+0.44*data_players2$FTA.x+data_players2$As.x+data_players2$To.x)==0,0, data_players2$As.x/(data_players2$x2A.x+data_players2$x3A.x+0.44*data_players2$FTA.x+data_players2$As.x+data_players2$To.x))
```
...
```{r}
data_players2$Pomeroy_Assist_ratio <- data_players2$As.x/(((data_players2$Min / (data_players2$Min.x / 5)) * (data_players2$x3A.y+data_players2$x2A.y) ) - (data_players2$x3A.x+data_players2$x2A.x))
```
...
```{r}
data_players2$Steal_pct<-data_players2$St.x/(data_players2$Min/(data_players2$Min.x/5)*(data_players2$Poss))
```
...
```{r}
data_players2$Block_pct<-data_players2$Fv.x/(data_players2$Min/(data_players2$Min.x/5)*(data_players2$x2A_Opp))
```

```{r}
data_players2$ThreePointers_ratio <- ifelse((data_players2$x3A.x+data_players2$x2A.x)==0,0,data_players2$x3A.x/(data_players2$x3A.x+data_players2$x2A.x))

data_players_cluster <- data_players2[,c("PlayerName","Assist_pct","Turnover_pct","Offensive_Reb_pct","eFG_player","TS_player","Usage_rate","Points_per_possesion","ThreePointers_ratio","Defensive_Reb_pct","FTR_player","Hollinger_Assist_ratio","Pomeroy_Assist_ratio","Steal_pct","Block_pct")]
```
...
quinteto_ideal<-c("MICIC, VASILIJE","LARKIN, SHANE","DATOME, LUIGI","MIROTIC, NIKOLA","TAVARES, WALTER")
